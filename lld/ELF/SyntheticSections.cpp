...
static void gpuHash64(ArrayRef<uint8_t> Data, uint8_t *dest) {
  ...
  data_buffer_size = data_buffer_size - (data_buffer_size % (1024*1024));

  size_t hash_size = 1024;
  size_t nHashes = data_buffer_size / hash_size;
  ...
  // assert(fBinary != nullptr);
  // fseek(fBinary, 0, SEEK_END);
  // binary_size = ftell(fBinary);
  // rewind(fBinary);
  // binary_file = (unsigned char *)malloc((binary_size+1)*sizeof(unsigned char)); // Enough memory for file + \0
  // fread((void*)binary_file, binary_size, 1, fBinary); // Read in the entire file
  ...
  output_clbuffer = clCreateBuffer(context, CL_MEM_READ_WRITE, result_buffer_size, NULL, &err);
  assert(err == CL_SUCCESS);
  ...
  err |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &output_clbuffer);
  err |= clSetKernelArg(kernel, 2, sizeof(hash_size), &hash_size);
  assert(err == CL_SUCCESS);

  // err = 0;
  // err = clSetKernelArg(kernel2, 0, sizeof(cl_mem), &output_clbuffer);
  // err = clSetKernelArg(kernel2, 1, sizeof(cl_mem), &total_output_clbuffer);
  // err = clSetKernelArg(kernel2, 2, sizeof(result_buffer_size), &result_buffer_size);
  // assert(err == CL_SUCCESS);
...
//do stuff
  ...
  switch (Config->BuildId) {
  case BuildIdKind::Fast:
    printf("fast bytes: %zu\n", Buf.size());
    gettimeofday(&start, NULL);
    if ((Config->UseOpenCL)) {
      printf("Using OpenCL for Hash...\n");
      gpuHash64(Buf, HashBuf);
    } else {
      printf("Using CPU for hash...\n");
      computeHash(Buf, [](uint8_t *Dest, ArrayRef<uint8_t> Arr) {
        write64le(Dest, xxHash64(Arr));
      });
    }
    gettimeofday(&stop, NULL);
    printf("fast seconds: %f\n", (double)(stop.tv_usec - start.tv_usec) / 1000000 + (double)(stop.tv_sec - start.tv_sec));
    break;
  case BuildIdKind::Md5:
    printf("md5\n");
    computeHash(Buf, [](uint8_t *Dest, ArrayRef<uint8_t> Arr) {
